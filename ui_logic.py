...
def date_matches(event_date: date, preset: str, today: date) -> bool:
    ...
    if preset == "Idag": return event_date == today
    if preset == "I helgen":
        days_to_sat = (5 - today.weekday()) % 7
        saturday = today + timedelta(days=days_to_sat)
        return saturday <= event_date <= saturday + timedelta(days=1)
    days = DATE_FILTER_DAYS.get(preset)
    return True if days is None else event_date <= today + timedelta(days=days)
...
def event_period_matches(event, preset: str, today: date) -> bool:
    ...
    if preset == "Idag": return start <= today <= end
    if preset == "I helgen":
        days_to_sat = (5 - today.weekday()) % 7
        saturday = today + timedelta(days=days_to_sat)
